...
// Fixed row heights — used identically in legend and columns
const ROW = { time: 44, sky: 44, temp: 44 };

const Divider = () => (
    <div style={{ width: '100%', height: 1, background: T.border, flexShrink: 0 }} />
);

// Individual forecast column
function ForecastColumn({ data, isLast }) {
    const delta = data.gust && data.gust > data.windSpeed
        ? Math.round(data.gust - data.windSpeed)
        : null;
    const capColor = delta ? getCapColor(delta) : null;
    const windH = Math.min(Math.max(data.windSpeed * 5, 40), 80);
    const capH = delta ? Math.min(Math.max(delta * 4, 6), MAX_BAR - windH) : 0;
        ...
        <div style={{ flexShrink: 0, width: COL, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingRight: 8, borderRight: isLast ? 'none' : `1px solid ${T.border}` }}>
            ...
            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(10,132,255,0.1)', border: '1px solid rgba(10,132,255,0.25)', borderRadius: 10, padding: '6px 0', marginBottom: 6 }}>
                <div style={{ transform: `rotate(${rotation}deg)`, display: 'flex' }}>
                    <MoveUp style={{ width: 32, height: 32, color: '#0A84FF' }} />
                </div>
                <div style={{ fontSize: 18, color: 'rgba(235,235,245,0.5)', marginTop: 2 }}>{data.degree}°</div>
            </div>

            {/* Stacked bar */}
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: MAX_BAR + 40, paddingBottom: 10 }}>
                {delta && (
                    <>
            ...
            <Divider />
            <div style={{ height: ROW.time, display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
                <span style={{ fontSize: 16, fontWeight: 'bold', color: 'white' }}>{data.time}</span>
            </div>
            <Divider />
            <div style={{ height: ROW.sky, display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
                <span style={{ fontSize: 24 }}>{getWeatherIcon(data.condition)}</span>
            </div>
            <Divider />
            <div style={{ height: ROW.temp, display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
                <span style={{ fontSize: 16, color: 'rgba(235,235,245,0.6)' }}>{data.temp}°F</span>
            </div>
        </div>
    );
}
...
function DayGroup({ date, items }) {
    return (
        <div style={{ display: 'inline-flex', flexDirection: 'column', borderRight: `2px solid ${T.border}`, paddingRight: 16, marginRight: 16 }}>
            <div style={{ ...T.dayBadge, padding: '6px 14px', borderRadius: 12, fontSize: 16, fontWeight: 'bold', textAlign: 'center', marginBottom: 12 }}>
                {date}
            </div>
...
export default function Forecast({ forecastInfo }) {
    ...
    return (
        <div style={{ ...T.card, borderRadius: 16, overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.6)', marginBottom: 16 }}>
            ...
            {/* Gust color legend */}
            <div style={{ ...T.elevated, borderBottom: `1px solid ${T.border}`, padding: '10px 16px' }}>
                <div style={{ fontSize: 14, color: 'rgba(235,235,245,0.4)', textAlign: 'center', marginBottom: 8 }}>Wind Gusts</div>
                <div style={{ display: 'flex', justifyContent: 'space-evenly', alignItems: 'flex-start' }}>
                    {[
                        { color: '#30D158', label: 'Calm', sub: '(<4 mph)' },
                        { color: '#FF9F0A', label: 'Caution', sub: '(4–7 mph)' },
                        { color: '#FF453A', label: 'Danger', sub: '(>7 mph)' },
                    ].map(({ color, label, sub }) => (
                        <div key={label} style={{ display: 'flex', alignItems: 'flex-start', gap: 6 }}>
                            <div style={{ width: 12, height: 12, background: color, borderRadius: 2, flexShrink: 0, marginTop: 3 }} />
                            <div>
                                <div style={{ fontSize: 15, color: 'rgba(235,235,245,0.7)' }}>{label}</div>
                                <div style={{ fontSize: 14, color: 'rgba(235,235,245,0.4)' }}>{sub}</div>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div style={{ display: 'flex' }}>
                ...
                {/* Sticky legend */}
                <div style={{ flexShrink: 0, width: 52, background: '#2C2C2E', borderRight: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', paddingRight: 6, paddingLeft: 6, paddingTop: 8 }}>
                    <div style={{ height: 46 }} />
                    <div style={{ height: 85, display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                        <span style={{ fontSize: 14, color: 'rgba(235,235,245,0.5)' }}>Dir</span>
                    </div>
                    <div style={{ height: MAX_BAR + 40, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', alignItems: 'flex-end', paddingBottom: 10, paddingTop: 40 }}>
                        <div />
                        ...
                        <div style={{ fontSize: 14, color: '#0A84FF' }}>Wind</div>
                    </div>
                    <Divider />
                    <div style={{ height: ROW.time, display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                        <span style={{ fontSize: 14, color: 'rgba(235,235,245,0.5)' }}>Time</span>
                    </div>
                    <Divider />
                    <div style={{ height: ROW.sky, display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                        <span style={{ fontSize: 14, color: 'rgba(235,235,245,0.5)' }}>Sky</span>
                    </div>
                    <Divider />
                    <div style={{ height: ROW.temp, display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
